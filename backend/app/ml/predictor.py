...
FALLBACK_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), "model.pkl")
...
class LoanPredictorEngine:
    ...
    def predict(self, input_data: dict) -> dict:
        ...
        Supports all 5 loan types with type-specific rules from LOAN_CONFIG.
        """
        # Resolve loan type (strips emoji prefix if present)
        loan_type_raw = (
        ...
        # Tenure: new forms send years, legacy sends months
        ...
        # Loan-type specific interest rate (auto, never user-input)
        ...
        if self.model_loaded and self.artifacts and "model" in self.artifacts:
            try:
                ...
                logger.error(f"ML Model inference error: {ex}. Using heuristic.")
                ml_prob = 0.5
        ...
        # Suggested max loan within type ceiling
        ...
        if approved:
            ...
            reason = (
                f"CIBIL score ({int(cibil)}) meets {loan_type} threshold, "
                f"DTI ratio ({int(dti_ratio*100)}%) is within the 50% cap."
            )
        else:
            recommendation = (
                f"{loan_icon} {loan_type} flagged: CIBIL {int(cibil)} or DTI {int(dti_ratio*100)}% exceeds thresholds. "
                f"Consider applying for Rs.{suggested_max:,.0f} or improving your credit score."
            )
            reason = (
                f"CIBIL score ({int(cibil)}) below {loan_type} benchmark ({min_cibil}+) or "
                f"total monthly obligation (Rs.{total_obligation:,.0f}) exceeds 50% of income."
            )

        tips = []
        if cibil < 750:
            tips.append("Pay all EMIs and credit card bills on time to boost CIBIL above 750.")
        if credit_usage > 0.3:
            tips.append(f"Lower credit card utilization from {int(credit_usage*100)}% to under 30%.")
        if dti_ratio > 0.4:
            tips.append("Pay down existing loans to reduce Debt-to-Income ratio below 40%.")
        if bank_balance < (loan_amt * 0.25):
            tips.append("Increase liquid savings to at least 25% of the loan amount.")
        if prev_defaults > 0:
            tips.append("Resolve all defaulted accounts with the credit bureau before reapplying.")
        if not tips:
            tips.append("Maintain your excellent financial track record and keep debt utilization low.")
